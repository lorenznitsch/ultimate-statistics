...
function WinLossRow({ label, wins, losses }: { label: string; wins: number; losses: number }) {
  ...
  return (
    <div className="flex items-center gap-3 py-1">
      <span className="w-20 text-sm text-gray-600 truncate">{label}</span>
...
export default function StatsPage() {
  ...
  function gameLabel(g: Game, teamName: string) {
    const isHome = g.home_base === teamName;
    ...
    <div className="space-y-6">
      ...
      {stats && !loading && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* 3. Häufigster Gegner */}
          <StatCard title="3 · Häufigster Gegner">
            {stats.mostFrequent ? (
              <div>
          ...
          {/* 4. Angstgegner */}
          <StatCard title="4 · Angstgegner (min. 2 Spiele)">
            {stats.worstAgainst ? (
              <div>
          ...
          {/* 5. Lieblingsgegner */}
          <StatCard title="5 · Lieblingsgegner (min. 2 Spiele)">
            {stats.bestAgainst ? (
              <div>
          ...
          <StatCard title="6 · Höchster Sieg">
            {stats.biggestWin ? (
              <div>
                <div className="text-lg font-bold text-green-600">{gameLabel(stats.biggestWin, team)}</div>
          ...
          <StatCard title="7 · Höchste Niederlage">
            {stats.biggestLoss ? (
              <div>
                <div className="text-lg font-bold text-red-500">{gameLabel(stats.biggestLoss, team)}</div>
              ...
              <ResponsiveContainer width="100%" height={140}>
                <BarChart data={stats.byYear} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                  ...
                  <Bar dataKey="count" radius={[4, 4, 0, 0]}>
                    {stats.byYear.map((_, i) => (
                      <Cell key={i} fill="#006B5E" />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
          ...
          {/* 11. Universe Points */}
          <StatCard title="11 · Universe Points (1-Punkt-Spiele)">
            <div className="flex justify-around text-center">
              <div>
